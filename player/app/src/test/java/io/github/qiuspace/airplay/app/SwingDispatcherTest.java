...
class SwingDispatcherTest {
    ...
    @Test
    void waitsForSynchronousUiCallbackToFinish() throws Exception {
        AtomicBoolean callbackFinishedOnEdt = new AtomicBoolean();

        SwingDispatcher.dispatchAndWait(() ->
                callbackFinishedOnEdt.set(SwingUtilities.isEventDispatchThread()));

        assertTrue(callbackFinishedOnEdt.get());
    }
}
