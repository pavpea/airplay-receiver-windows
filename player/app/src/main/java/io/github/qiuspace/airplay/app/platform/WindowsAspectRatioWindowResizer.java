...
public final class WindowsAspectRatioWindowResizer implements AutoCloseable {
    ...
    private final Runnable userInteractionFinished;
    private HWND handle;
    private Pointer previousWindowProc;
    private WindowProc windowProc;
    private volatile boolean installed;
    private volatile boolean userSizing;
    ...
    private WindowsAspectRatioWindowResizer(
            Window window, Runnable userInteractionFinished) {
        this.window = window;
        this.userInteractionFinished = userInteractionFinished == null
                ? () -> {
                }
    ...
    public static WindowsAspectRatioWindowResizer install(Window window) {
        return install(window, null);
    }

    public static WindowsAspectRatioWindowResizer install(
            Window window, Runnable userInteractionFinished) {
        WindowsAspectRatioWindowResizer resizer =
                new WindowsAspectRatioWindowResizer(window, userInteractionFinished);
    ...
    /**
     * Updates the bounds Windows will restore after leaving maximized state
     * without changing the window's current maximized state.
     */
    public boolean updateNormalBounds(Rectangle currentLogicalBounds,
                                      Rectangle targetLogicalBounds) {
        if (currentLogicalBounds == null || targetLogicalBounds == null || !isNativeActive()) {
            return false;
        }
        try {
            ...
            if (!User32.INSTANCE.GetWindowPlacement(handle, placement).booleanValue()) {
                throw new IllegalStateException("GetWindowPlacement failed with error "
                        + Native.getLastError());
            }
            placement.read();
            ...
            AffineTransform transform = configuration == null
                    ? new AffineTransform()
                    : configuration.getDefaultTransform();
            Rectangle nativeNormalBounds = new Rectangle(
                    placement.rcNormalPosition.left,
                    placement.rcNormalPosition.top,
                    placement.rcNormalPosition.right - placement.rcNormalPosition.left,
                    placement.rcNormalPosition.bottom - placement.rcNormalPosition.top);
            Rectangle translated = translateNormalBounds(
                    nativeNormalBounds, currentLogicalBounds, targetLogicalBounds,
                    transform.getScaleX(), transform.getScaleY());
            placement.rcNormalPosition.left = translated.x;
            placement.rcNormalPosition.top = translated.y;
            placement.rcNormalPosition.right = translated.x + translated.width;
            placement.rcNormalPosition.bottom = translated.y + translated.height;
            placement.write();
            if (!User32.INSTANCE.SetWindowPlacement(handle, placement).booleanValue()) {
                throw new IllegalStateException("SetWindowPlacement failed with error "
                        + Native.getLastError());
            }
            return true;
        } catch (RuntimeException | LinkageError error) {
            log.warn("Could not update the playback window restore bounds", error);
            return false;
        }
    }
    ...
    /** Returns whether Windows is currently running an interactive move/resize loop. */
    public boolean isUserSizing() {
        return userSizing;
    }
    ...
    private void notifyUserInteractionFinished() {
        try {
            userInteractionFinished.run();
        } catch (Throwable error) {
            log.warn("Playback window user-interaction callback failed", error);
        }
    }
    ...
    static Rectangle translateNormalBounds(Rectangle nativeNormalBounds,
                                           Rectangle currentLogicalBounds,
                                           Rectangle targetLogicalBounds,
                                           double scaleX,
                                           double scaleY) {
        double usableScaleX = scaleX > 0 && Double.isFinite(scaleX) ? scaleX : 1d;
        double usableScaleY = scaleY > 0 && Double.isFinite(scaleY) ? scaleY : 1d;
        int x = nativeNormalBounds.x
                + (int) Math.round((targetLogicalBounds.x - currentLogicalBounds.x) * usableScaleX);
        int y = nativeNormalBounds.y
                + (int) Math.round((targetLogicalBounds.y - currentLogicalBounds.y) * usableScaleY);
        int width = Math.max(1, (int) Math.round(targetLogicalBounds.width * usableScaleX));
        int height = Math.max(1, (int) Math.round(targetLogicalBounds.height * usableScaleY));
